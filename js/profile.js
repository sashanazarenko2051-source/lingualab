...
App.profile = (function () {
  ...
  // Hue-rotate can't produce achromatic results (it just spins whatever hue
  // sepia() left behind), so white/gray/black swatches need a different
  // filter entirely: desaturate and push brightness toward the target
  // instead of rotating a hue that isn't meaningfully there.
  function tintFilter(hex) {
    const { h, s, l } = hexToHsl(hex);
    if (s < 0.22) {
      if (l > 0.9) return 'brightness(0) invert(1)';
      if (l < 0.12) return 'brightness(0)';
      return `grayscale(1) brightness(${(0.6 + l * 0.8).toFixed(2)})`;
    }
    const rotate = Math.round(h - 35);
